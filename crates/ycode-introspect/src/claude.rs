    ...
    for _ in 0..40 {
        ...
        if kind == "user" && first_user.is_none() {
            if let Some(text) = extract_text(&v) {
                let text = text.trim();
                // A slash-command turn carries the user's words inside
                // `<command-args>`. Pull them out rather than skipping the
                // block: `/goal 继续完成迁移计划` really is what this session
                // is about, and the alternative title is the command's
                // expanded prompt template — the same paragraph for every
                // session that used that command.
                if let Some(args) = command_args(text) {
                    first_user = Some(truncate_title(args));
                } else if !text.is_empty() && !is_injected_preamble(text) {
...
/// The user's argument to a slash command, when this message is the CLI's
/// `<command-name>…<command-args>` envelope and the argument isn't empty.
...
/// Text the CLI wraps around the user's actual first message. Claude Code
/// types all of these as `user`, so a naive "first user message" reads the
/// wrapper instead of the prompt — and every session that opened with a slash
/// command ends up sharing one title.
///
/// `<local-command-…>` is matched by prefix rather than by listing each tag:
/// stdout, stderr and caveat all exist, and the set grows with the CLI.
///
/// The prompt a command expands *into* is not listed here — it is ordinary
/// prose and can't be pattern-matched. `command_args` above sidesteps that by
/// taking the title from the command envelope, which comes first.
...
mod tests {
    ...
    #[test]
    fn title_skips_the_cli_injected_preamble() {
        // Claude Code wraps a slash-command turn in its own `user` messages.
        // Reading those as the title made every such session show up as
        // "<local-command-caveat>Caveat: The messages…" — four identical rows
        // in the sidebar for four unrelated conversations.
        assert!(is_injected_preamble(
            "<local-command-caveat>Caveat: The messages below were generated…"
        ));
        // stdout is the one that actually bit: `/model` writes its
        // confirmation here, so every session that started by switching
        // models was titled "Set model to …".
        assert!(is_injected_preamble(
            "<local-command-stdout>Set model to `Fable 5`</local-command-stdout>"
        ));
        assert!(is_injected_preamble("<local-command-stderr>boom"));
        assert!(is_injected_preamble("<command-name>/commit</command-name>"));
        assert!(is_injected_preamble(
            "Caveat: The messages below were generated by the user while running local commands."
        ));
        assert!(!is_injected_preamble("修复右侧终端拆分显示空白问题"));

        // The command envelope yields its argument instead of being dropped:
        // `/goal 继续完成迁移计划` is a better title than the paragraph that
        // command expands into, which is identical across sessions.
        assert_eq!(
            command_args(
                "<command-name>/goal</command-name>\n  <command-message>goal</command-message>\n  <command-args>继续完成迁移计划</command-args>"
            ),
            Some("继续完成迁移计划")
        );
        // A bare command has no argument worth showing.
        assert_eq!(
            command_args("<command-name>/clear</command-name>\n<command-args></command-args>"),
            None
        );
        assert_eq!(command_args("一句普通的话"), None);
        // A mention of the tag mid-sentence is the user talking about it.
        assert!(!is_injected_preamble(
            "为什么 transcript 里会有 <local-command-caveat>?"
        ));
    }
